...
export function TaskCalendar({ tasks }: { tasks: Task[] }) {
  ...
  return <Surface style={styles.card} elevation={0}>
    ...
    <View style={styles.selection}><Text style={styles.selectionDate}>{longDate(selected)}</Text><Text style={styles.selectionCopy}>{selectedItems.length ? `${selectedItems.length} scheduled task${selectedItems.length === 1 ? "" : "s"}` : "Nothing scheduled"} · {completed.length} completed</Text>{selectedItems.slice(0, 3).map((task) => <Text key={task.id} numberOfLines={1} style={styles.taskLine}>{task.completions.includes(selectedKey) ? "✓" : "•"} {task.title}</Text>)}</View>
  </Surface>;
}
...
const makeStyles = (colors: { surface: string; line: string; ink: string; muted: string; primary: string; accent: string; primarySoft: string; canvas: string }) => StyleSheet.create({
  ...
  selection: { backgroundColor: colors.canvas, borderRadius: 14, padding: 12, marginTop: 14 }, selectionDate: { color: colors.ink, fontWeight: "800" }, selectionCopy: { color: colors.muted, marginTop: 2, fontSize: 12 }, taskLine: { color: colors.ink, marginTop: 7, fontSize: 13 },
});
