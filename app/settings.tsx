...
export default function SettingsScreen() {
  ...
  const initial = user?.email?.trim().charAt(0).toUpperCase() || "D";
  ...
  return <AppScreen><View style={styles.top}><IconButton icon="arrow-left" accessibilityLabel="Back" onPress={() => router.back()} /><Text style={styles.eyebrow}>SETTINGS</Text></View><Text variant="headlineMedium" style={styles.heading}>Your space</Text><Text style={styles.lead}>Keep the app comfortable and your account in your control.</Text>
    <View style={styles.layout}>
      <Surface style={styles.rail} elevation={0}><View style={styles.avatar}><Text style={styles.avatarText}>{initial}</Text></View><MaterialCommunityIcons name="account-outline" size={21} color={colors.primary} /><MaterialCommunityIcons name="palette-outline" size={21} color={colors.muted} /><MaterialCommunityIcons name="shield-check-outline" size={21} color={colors.muted} /></Surface>
...
const makeStyles = (colors: { canvas: string; surface: string; line: string; ink: string; muted: string; primary: string; primarySoft: string; danger: string }) => StyleSheet.create({
  ...
  layout: { flexDirection: "row", gap: 12 }, rail: { width: 68, alignItems: "center", gap: 20, paddingVertical: 16, borderRadius: 20, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface }, avatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center" }, avatarText: { color: "#FFFFFF", fontWeight: "800" }, content: { flex: 1, gap: 12, minWidth: 0 },
